...
import com.sky.weixin.contacts.util.MDepartment;
import com.sky.weixin.menu.util.Menu;
...
//	        	e.printStackTrace();
	            accessToken = null;  
	            // 获取token失败  
	            String error = String.format("!!!!! 获取token失败,请检查CorpId和Secret是否正确。 errcode:{%s} errmsg:{%s}", jsonObject.getInt("errcode"), jsonObject.getString("errmsg"));
    ...
    public static String PostMessage(String access_token ,String RequestMt , String RequestURL , String outstr){
    	...
    	JSONObject jsonobject = WeixinUtil.HttpRequest(RequestURL, RequestMt, outstr);
    	 if (null != jsonobject) {
 	        if (0 != jsonobject.getInt("errcode")) {  
 	            result = jsonobject.getInt("errcode");  
 	            error = String.format("errcode:{%s} errmsg:{%s}", jsonobject.getInt("errcode"), jsonobject.getString("errmsg"));  
// 	            System.out.println(error); 
 	        }  
 	    }
    	return error;
     ...
     * @param outstr 提交json数据
     * */
    public static String GetDepartmentList(String access_token ,String RequestMt , String RequestURL , String outstr){
    	int result = 0;
    	String error =null;
    	RequestURL = RequestURL.replace("ACCESS_TOKEN", access_token);
    	JSONObject jsonobject = WeixinUtil.HttpRequest(RequestURL, RequestMt, null);
    	if (null != jsonobject) {
    		if (0 != jsonobject.getInt("errcode")) {
    			result = jsonobject.getInt("errcode");  
    			error = String.format("errcode:{%s} errmsg:{%s}", jsonobject.getInt("errcode"), jsonobject.getString("errmsg"));  
// 	            System.out.println(error); 
    			...
    			MDepartment rror=(MDepartment)JSONObject.toBean((JSONObject)((JSONArray)jsonobject.get("department")).get(0),MDepartment.class);
    			error=String.format("department:{%s}", jsonobject.getString("department"));
    		}
    	}
    	return error;
    }
